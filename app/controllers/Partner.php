...
class Partner extends Controller
{
	...
	{
		$target_dir = $_SERVER['DOCUMENT_ROOT'].'/valiant_native/public/image/partner/';
		...
		if(isset($_POST["submit"])) {
		  ...
		  if($check !== false) {
		    ...
		    Flasher::setFlash('Partner gagal ditambahkan, pastikan file gambar anda benar.', 'danger');
		    $uploadOk = 0;
		  }
		}
		...
		if ($_FILES["avatar"]["size"] > 2000000) {
		  Flasher::setFlash('Partner gagal ditambahkan, file gambar terlalu besar. Pastikan ukuran file dibawah 2 MB.', 'danger');
		  header('Location: '. BASEURL . '/Admin/pg_partner/');
		  $uploadOk = 0;
		}
		...
		// if everything is ok, try to upload file
		  ...
		  if (move_uploaded_file($_FILES["avatar"]["tmp_name"], $target_file)) {
		  	if ($this->model('Partner_model')->tambah($_POST, $name_file) > 0) {
				Flasher::setFlash('Partner Behasil Ditambahkam.', 'success');
				header('Location: '. BASEURL . '/Admin/pg_partner/');
				exit;
			}else{
				Flasher::setFlash('Partner gagal ditambahkan.', 'danger');
	...
	public function ubahProfile(){
		if ($this->model('Partner_model')->ubahProfile($_POST) > 0) {
			Flasher::setFlash('Informasi berhasil diperbarui.', 'success');
			header('Location: '. BASEURL . '/Admin/pg_detailpartner/'.$_POST['id_partner'] );
			exit;
		}else{
	...
	public function ubahFotoPartner(){
		$target_dir = $_SERVER['DOCUMENT_ROOT'].'/valiant_native/public/image/team/Partner/';
		...
		// if everything is ok, try to upload file
		  ...
		  if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
		  	if ($this->model('Team_model')->ubahFotoPartner($_POST, $name_file) > 0) {
	...
	public function hapus(){
		if ($this->model('Partner_model')->hapus($_POST) > 0) {
			Flasher::setFlash('Partner sukses dihapus', 'success');
			header('Location: '. BASEURL . '/Admin/pg_partner/');
			exit;
		}else{
